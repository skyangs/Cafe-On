...
import com.example.order.global.exception.exception.CustomException;
import com.example.order.global.exception.exception.ErrorCode;
...
public class MemberService {
  ...
  public Member signUp(SignUpRequest request) {
    ...
    if (member.isPresent()) {
      throw new CustomException(ErrorCode.ALREADY_EXIST_MEMBER_ID);
    }

    Member newMember = Member.of(request.memberId(), request.password(), request.name(),
        request.authType(), request.phoneNum(), request.grade());
